...
print 'COMPUTING sigma-b LIKELIHOOD MAP'
sigma_array=10**arange(-3.,3.,0.1)
b_array=arange(-10.,10.,0.1)
print 'len(X)',len(X)
Nsigma=len(sigma_array)
Nb=len(b_array)
Xg, Yg=np.meshgrid(array(b_array), log10(array(sigma_array)))
Z = Xg*Yg
max_val=0.
print 'b',b
for j in range(0,Nb):
  ...
  for i in range(0,Nsigma):
    val = kelly_ll([sigma_array[i], tau, b_array[j]], t, X_data, err)
    ll.append(val)
    Z[i][j]=val
...
pcolor(Yg, Xg, Z)
cbar=colorbar()
cbar.ax.set_ylabel(r'$2\ln(p(Y|\sigma,\tau, b)$)', rotation=270)
levels=arange(-max_val,max_val,0.1*max_val)
contour(Yg, Xg, Z, levels=levels, colors='k')
plot([log10(sigma)],[b],'wo', mew=2, mfc='none', mec='w')
ylim(min(b_array),max(b_array))
...
# tau-b likelihood map
print 'COMPUTING tau-b LIKELIHOOD MAP'
tau_array=10**arange(-5.,5.,0.1)
b_array=arange(-10.,10.,0.1)
print 'len(X)',len(X)
Ntau=len(tau_array)
Nb=len(b_array)
Xg, Yg=np.meshgrid(array(b_array), log10(array(tau_array)))
Z = Xg*Yg
max_val=0.
print 'b',b
for j in range(0,Nb):
  ...
  for i in range(0,Ntau):
    val = kelly_ll([sigma, tau_array[i], b_array[j]], t, X_data, err)
    ll.append(val)
    Z[i][j]=val
  max_val=max(max_val,max(ll))
  #if(j<=6): semilogx(tau_array,ll, '+-',lw=2, label='$\sigma$=%1.2e'%(sigma_array[j]))
...
levels=arange(-max_val,max_val,0.1*max_val)
contour(Yg, Xg, Z, levels=levels, colors='k')
plot([log10(tau)],[b],'wo', mew=2, mfc='none', mec='w')
ylim(min(b_array),max(b_array))
