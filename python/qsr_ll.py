...
def hojjati_ll(time_array, flux_array, ph_err_array, sig, tau, b):
  ...
  ll=-array(matrix(flux_array[0:Nsamp])*matrix(M_inv)*matrix(flux_array[0:Nsamp]).T)[0][0]-log_M_det-Nsamp*log(2.*pi)
...
def kelly_ll(time_array, flux_array, ph_err_array, sig, tau, b):
  ...
  Omega=[tau*sig**2/2.]
  a=[0.]
  ll=-(x_hat[0] - x_star[0])**2/(Omega[0]+ph_err_array[0]**2) - log(2*pi*(Omega[0]+ph_err_array[0]**2))
  print sig, tau
