...
for fnm in data_file_list:
  ...
  num_seasons =  len([s for s in ls if fnm in s and 'season' in s])
    ...
    for j in range(i+1,len(image_list)):
      ...
      for ph_un_add in ph_un_add_list:
        for season in range(1,num_seasons+1):
          com = './fot_delay.py -i %s_season%d.rdb -l %s -m %s -su %1.3f -dtp 0. -dtpmin -200. -dtpmax 200. -dmp 0.5 -dmpmin -5. -dmpmax 5. -sp 0.07 -spmin 0.00007 -spmax 7. -tp 121. -tpmin 10. -tpmax 1000. -mp -13. -mpmin -30 -mpmax 100. -z 0. -o %s_season%d_%s_%s_%s > %s_season%d_%s_%s_%s.log &'%(fnm, season, image_list[i], image_list[j], float(ph_un_add.replace('p','.')),fnm, season, image_list[i], image_list[j], ph_un_add, fnm, season, image_list[i], image_list[j], ph_un_add)
          print ''
