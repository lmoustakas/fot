...
\label{eq:generative}
\end{equation} 
where $dB(s)$ is a temporally uncorrelated normally distributed random variable with zero mean and variance $dt$.
...
\over{p({\mathbf  d})}}. \nonumber 
\end{equation}
where the data observables {\bf d} are a collection of observed magnitudes ${x_{i}}$ with 
...
$X_1 = \{(t_{1,i},x_{1,i},\sigma_{1,i})\}$ and $X_2 = \{(t_{2,i},x_{2,i},\sigma_{2,i})\}$  
we map the points in $X_2$ to $X'_2 = \{(t_{2,i}-\Delta t,x_{2,i}-\Delta m,\sigma_{2,i})\}$  
such that the merged data $X_1$ and $X'_2$, denoted by $M(\Delta t, \Delta m ; X_1, X_2)$ is best described by a single light curve model.
The likelihood for the merged lightcurves under a hypothesis $(\Delta t, \Delta m)$ and ($\langle m\rangle$, $\tau$, and $\sigma$) is given by
\begin{align}
...
parameters ($\Delta t$, $\Delta m$, $\langle m\rangle$), including their
